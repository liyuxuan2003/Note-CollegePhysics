...
\subsubsection{匀速圆周运动}
    \setcounter{equation}{0}
    匀速圆周运动中的匀速表示线速度$\veb{v}$的大小为一定值。\\[3mm]
            ...
            \subfigure[速度示意图]
            {
                \begin{minipage}[t]{0.27\linewidth}
                    \begin{tikzpicture}[scale=0.9]
                        ...
                        \tkzLabelSegment[right](O,A){$\veb{v_a}=\veb{v}$}
            ...
            \subfigure[位移示意图]
            {
                \begin{minipage}[t]{0.36\linewidth}
                    \begin{tikzpicture}[scale=0.9]
                        ...
                        \tkzDrawSegment[-latex,add=0 and -0.4](A,a)
                        \tkzDrawSegment[-latex](B,b)

                        \tkzDefShiftPoint[a](0,-1.12){a2}
            ...
            \subfigure[速度示意图]
            {
                \begin{minipage}[t]{0.342\linewidth}
                    \begin{tikzpicture}[scale=0.9]
                        ...
                        \tkzLabelSegment[right=2pt](O,A){$\veb{v_a}=\veb{v}$}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=0.8]
                ...
                \tkzDrawSegments[-latex](O,v O,t O,n O,a)
                \tkzDrawSegments[dashed](t,a n,a)

                \tkzMarkAngle[size=0.443](v,O,a)

                \tkzDefPoint(0,-4){O'}
                \tkzDrawArc[R,dashed,color=MediumGray](O',4cm)(20,160)

                \tkzLabelPoint[right](n){$\veb{a_n}$}
                \tkzLabelPoint[above](t){$\veb{a_t}$}
                \tkzLabelPoint[above](v){$\veb{v}$}
                \tkzLabelPoint[left](a){$\veb{a}$}

                \tkzDefShiftPoint[O](-0.7,-0.3){Ot}
                \node at(Ot){$\theta$};
            \end{tikzpicture}
            \caption{总加速度的示意图}
        \end{center}
...
\subsection{力}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                \draw (-0.5,-0.3) rectangle (0.5,0.3);
...
\subsubsection{共点力的合成}
    \setcounter{equation}{0}
    已知两个力$\veb{F_1},\veb{F_2}$的大小，已知两者的夹角为$\theta$，现研究其合力的大小和方向。\\[3mm]
    考虑以下示意图：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                \tkzInit[xmin=-1.0,xmax=6.5,ymin=-0.5,ymax=4.5]
                \tkzClip

                \tkzDefPoint(0,0){O}
                \tkzDefShiftPoint[O](0:4){A1}
                \tkzDefShiftPoint[O](60:3){A2}
                \tkzDefShiftPoint[A1](60:3){B}

                \tkzDefLine[orthogonal=through B](O,A1)
                \tkzGetPoint{b1}

                \tkzInterLL(O,A1)(b1,B)
                \tkzGetPoint{B1}

                \tkzDefLine[orthogonal=through B](O,A2)
                \tkzGetPoint{b2}

                \tkzInterLL(O,A2)(B,b2)
                \tkzGetPoint{B2}

                \tkzDrawSegments[-latex](O,A1 O,A2 O,B)

                \tkzDrawSegments[dashed](A1,B1 A2,B2 B,B1 B,B2 B,A1 B,A2)

                \tkzMarkRightAngles[size=0.15](B,B2,A2 B,B1,A1)

                \tkzMarkAngle[size=0.3](A1,O,A2)
                \tkzMarkAngle[size=0.742](A1,O,B)
                \tkzMarkAngle[size=0.672](B,O,A2)

                \tkzMarkAngle[size=0.26](B,A1,O)
                ...
                \tkzLabelSegment[below](O,A1){$\veb{F_1}$}
                \tkzLabelSegment[left](O,A2){$\veb{F_2}$}
                \tkzLabelSegment[above](O,B){$\veb{F}$}

                \tkzDefShiftPoint[O](0.35,0.32){theta}

                \tkzDefShiftPoint[O](1.0,0.2){phi1}
                \tkzDefShiftPoint[O](0.7,0.6){phi2}

                \tkzDefShiftPoint[A1](-0.75,0.25){theta2}

                \node at(theta){$\theta$};
                \node at(phi1){$\varphi_1$};
                \node at(phi2){$\varphi_2$};

                \node at(theta2){$\pi-\theta$};
            \end{tikzpicture}
            \caption{共点力的合成}
        \end{center}
    \end{figure}\\
...
\subsubsection{拉密定理}
    \setcounter{equation}{0}
    \textbf{拉密定理：}当三个共点力平衡时，任意一个力与另外两个力夹角正弦的比值相同。\\[3mm]
    拉密定理的数学表达：
    \begin{large}
        \begin{equation*}
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            \subfigure[力的示意图]
            {
                \begin{tikzpicture}[scale=0.7]
                    \tkzDefPoint(0,0){O}
                    \tkzDefShiftPoint[O](37:3){F1}
                    \tkzDefShiftPoint[O](127:4){F2}
                    \tkzDefShiftPoint[O](-90:5){F3}
    
                    \tkzDrawSegments[-latex](O,F1 O,F2 O,F3)
                    \tkzMarkAngle[size=0.42](F1,O,F2)
                    \tkzMarkAngle[size=0.42](F2,O,F3)
                    \tkzMarkAngle[size=0.42](F3,O,F1)

                    \tkzDefShiftPoint[O](-0.6,-0.3){theta1}
                    \tkzDefShiftPoint[O](+0.7,-0.3){theta2}
                    \tkzDefShiftPoint[O](0,+0.7){theta3}

                    \node at(theta1){$\theta_1$};
                    \node at(theta2){$\theta_2$};
                    \node at(theta3){$\theta_3$};

                    \tkzLabelPoint[right](F1){$\veb{F_1}$}
                    \tkzLabelPoint[left](F2){$\veb{F_2}$}
                    \tkzLabelPoint[right](F3){$\veb{F_3}$}
            ...
            {
                \begin{tikzpicture}[scale=0.7]
                    \tkzInit[xmin=-3,xmax=+5,ymin=-2.7,ymax=6.1]
                    \tkzClip

                    \tkzDefPoint(0,0){A}
                    \tkzDefShiftPoint[A](37:3){B}
                    \tkzDefShiftPoint[A](37:3.1){B1}
                    \tkzDefShiftPoint[B](127:4){C}
                    \tkzDefShiftPoint[B](127:4.1){C1}
                    \tkzDefShiftPoint[O](-90:5.1){A1}
    
                    \tkzDrawSegments[-latex](A,B B,C C,A)
                    \tkzDrawSegments[dashed,add=-1.0 and 0.5](A,B B,C C,A)

                    \tkzMarkAngle[size=0.42](B1,B,C)
                    \tkzMarkAngle[size=0.42](C1,C,A)
                    \tkzMarkAngle[size=0.42](A1,A,B)

                    \tkzLabelSegment[below right](A,B){$\veb{F_1}$}
                    \tkzLabelSegment[above right](B,C){$\veb{F_2}$}
                    \tkzLabelSegment[left](C,A){$\veb{F_3}$}

                    \tkzLabelPoint[below right=6pt](A){$\theta_1$}
                    \tkzLabelPoint[above=7pt](B){$\theta_2$}
                    \tkzLabelPoint[left=7pt](C){$\theta_3$}
                \end{tikzpicture}
            }
            \caption{拉密定理的示意图}
        \end{center}
    \end{figure}\\
    如图所示，由于三个共点力平衡即矢量和为零，故三者的矢量可以首尾相接构成封闭三角形。\\[3mm]
    根据正弦定理和诱导公式：
    \begin{align}
        &\frac{F_1}{\sin{(\pi-\theta_1)}}=\frac{F_2}{\sin{(\pi-\theta_2)}}=\frac{F_3}{\sin{(\pi-\theta_3)}}\\[2mm]
        &\frac{F_1}{\sin{\theta_1}}=\frac{F_2}{\sin{\theta_2}}=\frac{F_3}{\sin{\theta_3}}
    \end{align}\\
    由此可见，拉密定理本质为正弦定理在三力平衡中的实际应用。
...
\subsubsection{牛顿第三定律}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                \draw (-3.4,+0.35) rectangle (-2.6,-0.35);
                \draw (+2.6,-0.35) rectangle (+3.4,+0.35);
                ...
                \tkzDrawSegments[-latex](A,FA B,FB)

                \tkzLabelPoint[right](FA){$\veb{F_{AB}}$}
                \tkzLabelPoint[left](FB){$\veb{F_{BA}}$}

                \tkzLabelPoint[below=10pt](A){$A$}
                \tkzLabelPoint[below=10pt](B){$B$}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                ...
                \tkzDrawSegments[-latex](A,f1 B,f2 A,F1 B,F2)

                \tkzLabelPoint[right](f1){$\veb{f_1}$}
                \tkzLabelPoint[left](f2){$\veb{f_2}$}
                \tkzLabelPoint[left](F1){$\veb{F_1}$}
                \tkzLabelPoint[right](F2){$\veb{F_2}$}

                \tkzLabelPoint[below=10pt](A){$m_1$}
                \tkzLabelPoint[below=10pt](B){$m_2$}
...
\subsubsection{弹力}
    ...
    \begin{figure}[h]
        \begin{center}
            \subfigure[轻杆可以提供拉力]
            {
                \begin{tikzpicture}[scale=0.75]
                    \tkzDefPoint(0,0){O}
                    \tkzDefShiftPoint[O](0,+1.7){N}
                    \tkzDefShiftPoint[O](0,-1.7){G}
    
                    \tkzDefShiftPoint[O](0,+0.3){H1}
                    \tkzDefShiftPoint[H1](0,+4.0){H2}

                    \tkzDrawCircle(O,H1)
    
                    \tkzDefShiftPoint[H1](-0.1,0){H1L}
                    \tkzDefShiftPoint[H1](+0.1,0){H1R}
                    \tkzDefShiftPoint[H2](-0.1,0){H2L}
                    \tkzDefShiftPoint[H2](+0.1,0){H2R}
    
                    \tkzDrawPolygon(H1L,H1R,H2R,H2L)
    
                    \tkzDefShiftPoint[H2](-1.6,0){L}
                    \tkzDefShiftPoint[H2](+1.6,0){R}
    
                    \tkzDrawSegment[ultra thick](L,R)
                    
                    \tkzDrawSegments[-latex](O,G O,N)
                    \tkzLabelPoint[right](G){$G$}
                    \tkzLabelPoint[right](N){$T$}
                    \tkzDrawPoints[fill=white](O)
                \end{tikzpicture}
            }\qquad
            \subfigure[轻绳可以提供拉力]
            {
                \begin{tikzpicture}[scale=0.75]
                    ...
                    \tkzDrawSegments[thick,color=MediumGray](H1,H2)
        ...
        \end{center}
    \end{figure}\\
    轻杆是一种没有质量的细长状物体，既可以提供拉力，也可以提供压力。\\[3mm]
    轻绳是一种没有质量的细长状物体，只可以提供拉力，不可以提供压力。\\[3mm]
    由于轻绳和轻杆均被假设为没有质量，若其受力不平衡，则牛顿第二定律指出其加速度无限大，从而导致不合理的结果，因此轻绳和轻杆的受力一定是平衡的。
...
\subsection{受力分析的实例}
    本章将分析一些常见情况下的物体受力和运动情况。

\subsubsection{斜面上的静止物体}
    \setcounter{equation}{0}
    现研究一个倾斜角为$\theta$的斜面上的静止物体的受力情况。\\[3mm]
    定义平行斜面向上的方向作为$x$轴的正方向，定义垂直斜面向上的方向作为$y$轴的正方向。\\[3mm]
    物体受力分析如图所示：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.15]
                ...
                \tkzDefPoint(6,0){T2}
                \tkzDefPoint(6,3){T3}
                \tkzDrawPolygon[color=DarkGray](T1,T2,T3)

                \tkzDefPoint(2.94,1.62){O}
                \tkzDefPoint(2.8,1.4){O1}
                \tkzDefPoint(3.2,1.6){O2}
                \tkzDefPoint(3.08,1.84){O3}
                \tkzDefPoint(2.68,1.64){O4}
                \tkzDrawPolygon[fill=LightGray](O1,O2,O3,O4)

                \tkzDefShiftPoint[O](26.57:1.5){X}
                \tkzDefShiftPoint[O](116.57:1.8){Y}
                \tkzDefShiftPoint[O](26.57:-1.5){X-}
                \tkzDefShiftPoint[O](116.57:-1.8){Y-}

                \tkzDrawSegments[color=MediumGray,dashed,add=1.0 and 0,-latex](O,X)
                \tkzDrawSegments[color=MediumGray,dashed,add=0.8 and 0,-latex](O,Y)

                \tkzDefShiftPoint[O](116.57:1.212){N}
                \tkzDefShiftPoint[O](-90:1.400){G}
                \tkzDefShiftPoint[O](26.57:0.700){f}

                \tkzDrawSegments[-latex](O,N O,G O,f)
                \tkzDrawPoint[fill=white](O)

                \tkzLabelPoint[left](N){$\veb{N}$}
                \tkzLabelPoint[right](G){$\veb{G}$}
                \tkzLabelPoint[above](f){$\veb{f}$}

                \tkzLabelPoint[above,color=MediumGray](X){$x$}
                \tkzLabelPoint[right,color=MediumGray](Y){$y$}

                \tkzMarkAngle[size=0.5](T2,T1,T3)
                \tkzMarkAngle[size=0.40](G,O,Y-)

                \tkzMarkRightAngle[size=0.15](T3,T2,T1)

                \tkzDefShiftPoint[T1](0.7,0.15){th1}
                \tkzDefShiftPoint[O](0.13,-0.55){th2}
...
\subsubsection{斜面上的下滑物体}
    \setcounter{equation}{0}
    现研究一个倾斜角为$\theta$的斜面上的下滑物体的受力情况和运动情况。\\[3mm]
    定义平行斜面向上的方向作为$x$轴的正方向，定义垂直斜面向上的方向作为$y$轴的正方向。\\[3mm]
    物体受力分析如图所示：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.15]
                ...
                \tkzDefShiftPoint[O](116.57:1.212){N}
                \tkzDefShiftPoint[O](-90:1.400){G}
                \tkzDefShiftPoint[O](26.57:0.500){f}
                ...
                \tkzDefPoint(5.5,3.50){a1}
                \tkzDefPoint(5.5,3.00){v1}

                \tkzDefPoint(5,3.25){a2}
                \tkzDefPoint(5,2.75){v2}

                \tkzDrawSegments[-latex](a1,a2 v1,v2)

                \tkzLabelPoint[left](a2){$\veb{a}$}
                \tkzLabelPoint[left](v2){$\veb{v}$}
            \end{tikzpicture}
...
\subsubsection{斜面上的上滑物体}
    \setcounter{equation}{0}
    现研究一个倾斜角为$\theta$的斜面上的上滑物体的受力情况和运动情况。\\[3mm]
    定义平行斜面向上的方向作为$x$轴的正方向，定义垂直斜面向上的方向作为$y$轴的正方向。\\[3mm]
    物体受力分析如图所示：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.15]
                ...
                \tkzDrawSegments[-latex](a1,a2)
                \tkzDrawSegments[-latex](v2,v1)

                \tkzLabelPoint[left](a2){$\veb{a}$}
                \tkzLabelPoint[right](v1){$\veb{v}$}
            \end{tikzpicture}
        ...
        \begin{center}
            \subfigure[最高点为起点的情况]
            {
                \begin{tikzpicture}[scale=0.65]
                    ...
                    \tkzDefPoint(-2.4,-1.8){A}
                    \tkzDrawSegments(P,A P,Q)
                    ...
                    \tkzMarkRightAngle[size=0.15](P,A,Q)
                    \tkzMarkAngle[size=0.65](A,P,Q)

                    \tkzDefShiftPoint[P](-0.2,-0.9){theta}
                    \node at(theta){$\theta$};

                    \tkzLabelSegment[left](P,A){$s$}
                    \tkzLabelSegment[right](P,Q){$h$}
                \end{tikzpicture}
            }\qquad\qquad
            \subfigure[最低点为终点的情况]
            {
                \begin{tikzpicture}[scale=0.65]
                    ...
                    \tkzDefPoint(-2.4,+1.8){A}
                    \tkzDrawSegments(A,Q P,Q)
                    \tkzDrawSegment[dashed](P,A)
                    ...
                    \tkzMarkRightAngle[size=0.15](P,A,Q)
                    \tkzMarkAngle[size=0.65](P,Q,A)

                    \tkzDefShiftPoint[Q](-0.2,+0.9){theta}
                    \node at(theta){$\theta$};

                    \tkzLabelSegment[left](A,Q){$s$}
                    \tkzLabelSegment[right](P,Q){$h$}
                \end{tikzpicture}
            }
        \end{center}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                ...
                \tkzDrawSegments[-latex](O,G O,f)

                \tkzLabelPoint[right](G){$\veb{G}~~~~G=mg$}
                \tkzLabelPoint[right](f){$\veb{f}~~~~f=kv$}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=0.85]
                \tkzInit[xmin=-3.2,xmax=+3.2,ymin=-2.2,ymax=+2.2]
                \tkzClip

                \draw[thick] (0,0) ellipse (3 and 2);

                \tkzDefPoint(-2.236,0){S}

                \tkzDefPoint(-2.432,-1.171){A1}
                \tkzDefPoint(-2.432,+1.171){A2}

                \tkzDefPoint(2.935,-0.414){B1}
                \tkzDefPoint(2.935,+0.414){B2}

                \tkzDrawSegments(S,A1 S,A2 S,B1 S,B2)
                ...
                \tkzInterLL(S,A1)(L1,L2)
                \tkzGetPoint{a1}

                \tkzInterLL(S,A2)(L1,L2)
                \tkzGetPoint{a2}

                \tkzInterLL(S,B1)(R1,R2)
                \tkzGetPoint{b1}

                \tkzInterLL(S,B2)(R1,R2)
                \tkzGetPoint{b2}

                \clip (0,0) ellipse (2.98 and 1.98);

                \tkzDrawPolygon[fill=LightGray](S,a1,a2)
                \tkzDrawPolygon[fill=LightGray](S,b1,b2)

                \tkzDrawPoint[fill=white](S)

                \tkzLabelPoint[left](S){$S_2$}
                \tkzLabelPoint[right=95pt](S){$S_1$}
            \end{tikzpicture}
            \caption{开普勒第二定律的示意图}
        \end{center}
    \end{figure}\\
    开普勒第二定律实际是行星绕太阳公转过程中角动量守恒的具体表现。\\[3mm]
    开普勒第二定律对于一般的圆锥曲线轨道，例如抛物线轨道和双曲线轨道也是普遍适用的。
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                \tkzDefPoint(0,0){O}
                \tkzDefShiftPoint[O](15:1.75){V1}
                \tkzDefShiftPoint[O](105:2.5){V2}

                \tkzDrawCircle[color=MediumGray,dashed](O,V1)
                \tkzDrawCircle[color=MediumGray,dashed](O,V2)

                \tkzDrawSegments(O,V1 O,V2)

                \tkzDrawPoints[fill=white](O,V1,V2)

                \tkzLabelPoint[right](V1){$v_1$}
                \tkzLabelPoint[above](V2){$v_2$}
                
                \tkzLabelPoint[below](O){$M$}

                \tkzLabelPoint[below=25pt](O){$v_1>v_2$}
            \end{tikzpicture}
            \caption{轨道半径和线速度的关系}
        \end{center}
    \end{figure}\\
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                ...
                \tkzLabelPoint[right](V1){$\omega_1$}
                \tkzLabelPoint[above](V2){$\omega_2$}
                
                \tkzLabelPoint[below](O){$M$}

                \tkzLabelPoint[below=25pt](O){$\omega_1>\omega_2$}
            \end{tikzpicture}
            \caption{轨道半径和角速度的关系}
        \end{center}
    \end{figure}\\
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                ...
                \tkzLabelPoint[right](V1){$T_1$}
                \tkzLabelPoint[above](V2){$T_2$}
                
                \tkzLabelPoint[below](O){$M$}

                \tkzLabelPoint[below=25pt](O){$T_1<T_2$}
            \end{tikzpicture}
            \caption{轨道半径和周期的关系}
        \end{center}
    \end{figure}\\
...
\subsubsection{天体质量的测量}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                \tkzDefPoint(0,0){O}
                \tkzDefShiftPoint[O](180:0.7){M}
                \tkzDefShiftPoint[O](0:2.5){R}

                \tkzDrawCircle[thick,fill=LightGray](O,M)
                \tkzDrawCircle[color=MediumGray,dashed](O,R)
                
                \tkzDrawSegments(O,R)

                \tkzLabelSegment[above](O,R){$r$}

                \tkzDrawPoints[fill=white,size=3pt](O,R)
                \tkzLabelPoint[below](O){$M$}
            \end{tikzpicture}
            \caption{天体质量测量的示意图}
        \end{center}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                ...
                \tkzLabelSegment[above,pos=0.45](O,M){$r_0$}
                \tkzLabelSegment[above](O,R){$r$}

                \tkzDrawPoints[fill=white,size=3pt](O,R,M)
                \tkzLabelPoint[below](O){$\rho$}
            \end{tikzpicture}
            \caption{天体密度测量的示意图}
        \end{center}
    \end{figure}\\
        ...
        \begin{center}
            \begin{tikzpicture}[scale=0.9]
                ...
                \foreach \i in {0,15,30,...,345}
                {
                    \tkzDefPoint(\i:1.5){A1}
                    \tkzDefPoint(\i:3.0){A2}
                    \tkzDrawSegment[-latex](O,A1)
                    \tkzDrawSegment(A1,A2)
                }

                \tkzDefPoint(0.3,0){R}
                \tkzDrawCircle[fill=white](O,R)
...
\subsubsection{电偶极子}
    电偶极子由指的是由两个的点电荷构成的结构，两者电量相等，同时符号相反。\\[3mm]
    电偶极子的示意图：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                ...
                \tkzDefShiftPoint[Q1](+0.3,0){Q1A}
                \tkzDefShiftPoint[Q2](-0.3,0){Q2A}

                \tkzDrawSegment[dashed,->](A1,A2)
                \tkzDrawSegment[dashed,->](B1,B2)

                \tkzDrawSegment[-latex,thick](Q1A,Q2A)

                \tkzDrawCircle[fill=white](Q1,Q1A)
                \tkzDrawCircle[fill=white](Q2,Q2A)

                \tkzLabelPoint[above left](Q2A){$\veb{l}$}
                \tkzLabelPoint[above=8pt](Q1){$-q$}
                \tkzLabelPoint[above=8pt](Q2){$+q$}
    ...
    \begin{large}
        ...
        \end{equation*}
    \end{large}\\
    其中$\veb{l}$称为电偶极子的轴，为一个由负电荷指向正电荷的矢量，没有单位。\\[8mm]
    通常我们会研究场点在以下两个位置时的电场情况：\\[3mm]
    1.场点位于电偶极子的轴的延长线上，即位于$A$轴上，此时场点通常记作$A$。\\[3mm]
    2.场点位于电偶极子的轴的中垂线上，即位于$B$轴上，此时场点通常记作$B$
    ...
    \begin{figure}[h!]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                ...
                \tkzDefShiftPoint[Q2](+1.5,0){A}
                \tkzDefShiftPoint[A](+1.5,0){E+}
                \tkzDefShiftPoint[A](-0.5,0){E-}

                \tkzDefShiftPoint[A](0,-0.6){AE1}
                \tkzDefShiftPoint[AE1](+1.0,0){AE2}

                \tkzDrawSegment[-latex](A,E+)
                \tkzDrawSegment[-latex](A,E-)

                \tkzDrawSegment[dashed](A,AE1)
                \tkzDrawSegment[-latex](AE1,AE2)

                \tkzLabelPoint[above](E+){$\veb{E^{+}}$}
                \tkzLabelPoint[above](E-){$\veb{E^{-}}$}

                \tkzLabelPoint[right](AE2){$\veb{E_A}$}
                \tkzLabelPoint[below left](O){$O$}
                
                \tkzLabelSegment[pos=0.5,above](O,Q1){$\dfrac{l}{2}$}
                \tkzLabelSegment[pos=0.5,above](O,Q2){$\dfrac{l}{2}$}                

                \tkzDrawPoint[fill=white](A)
                \tkzDrawPoint[fill=white](AE1)
            \end{tikzpicture}
            \caption{场点$A$在中心点右侧且在电偶极子外的情况}
        \end{center}
    \end{figure}\\
    场点$A$在中心点左侧时的电场强度情况：
    \begin{figure}[h!]
        ...
        \end{center}
    \end{figure}\\
    由于通常我们不关心电偶极子间的情况，因此我们此处只研究电偶极子外的情况。\\[3mm]
    当场点$A$在中心点$O$的右侧时有$E^{+}>E^{-}$，故场点$A$处的场强的大小为：$E_A=E^{+}-E^{-}$。\\[3mm]
    当场点$A$在中心点$O$的左侧时有$E^{-}>E^{+}$，故场点$A$处的场强的大小为：$E_A=E^{-}-E^{+}$。
    ...
    \begin{figure}[h!]
        \begin{center}
            \begin{tikzpicture}
                ...
                \tkzDefShiftPoint[Q1](0,-5.5){Q1A}
                \tkzDefShiftPoint[Q1](0,+5.5){Q1B}

                \tkzDefShiftPoint[Q2](0,-5.5){Q2A}
                \tkzDefShiftPoint[Q2](0,+5.5){Q2B}

                \tkzLabelPoint[below left](Q1){$-q$}
                \tkzLabelPoint[below right](Q2){$+q$}

                \tkzLabelPoint[above](X2){$r$}
                \tkzLabelPoint[right](Y2){$\veb{E_A}$}

                \draw[smooth,thick,samples=100,domain=+1.10:+5] plot(\x,{0.5*((+2*2*\x)/(\x*\x-0.5)^2)});
                \draw[smooth,thick,samples=100,domain=0:+0.67] plot(\x,{0.5*(-1/(+\x-1)^2-1/(+\x+1))});
                \draw[smooth,thick,samples=100,domain=-0.67:0] plot(\x,{0.5*(-1/(-\x-1)^2-1/(-\x+1))});
                \draw[smooth,thick,samples=100,domain=-5:-1.10] plot(\x,{0.5*((-2*2*\x)/(\x*\x-0.5)^2)});

                \draw[smooth,gray,thick,samples=100,domain=+0.75:+5] plot(\x,{+0.5*((2*2)/(\x)^3)});
                \draw[smooth,gray,thick,samples=100,domain=-5:-0.75] plot(\x,{-0.5*((2*2)/(\x)^3)});

                \tkzDrawSegment[dashed](Q1A,Q1B)
                \tkzDrawSegment[dashed](Q2A,Q2B)

                \tkzDrawPoint[fill=white](Q1)
                \tkzDrawPoint[fill=white](Q2)

                \tkzLabelPoint[below left](O){$O$}
            \end{tikzpicture}
            \caption{电偶极子轴线的延长线上的电场强度的函数曲线}
        \end{center}
    \end{figure}\\
    此处的近似假设为$r\gg l$，故可以近似认为$\left(r^2+\frac{l}{4}\right)^2=r^4$从而得到近似值。\\[3mm]
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                ...
                \tkzDefPoint(0,3){B}
                \tkzDefShiftPoint[B](+135:1){E+}
                \tkzDefShiftPoint[B](-135:1){E-}
                \tkzDefShiftPoint[B](180:1.414){E}
                ...
                \tkzDrawSegment[dashed](Q1,B)
                \tkzDrawSegment[dashed](Q2,B)
                
                \tkzDrawSegment[-latex](B,E+)
                \tkzDrawSegment[-latex](B,E-)
                \tkzDrawSegment[-latex](B,E)

                \tkzMarkAngle[size=0.491](B,Q2,Q1)
                \tkzMarkAngle[size=0.491](E+,B,Q1)
                ...
                \tkzLabelPoint[left](E+){$\veb{E^{+}}$}
                \tkzLabelPoint[left](E-){$\veb{E^{-}}$}
                \tkzLabelPoint[left](E){$\veb{E_B}$}

                \tkzLabelSegment[right](O,B){$r$}
                \tkzLabelSegment[pos=0.4,above](O,Q2){$\dfrac{l}{2}$}

                \node at (Q1) {$-$};
                \node at (Q2) {$+$};

                \node at(-0.6,+3.25){$\theta$};
                \node at(-0.6,+2.75){$\theta$};
                \node at(+2.3,+0.2){$\theta$};
            \end{tikzpicture}
            \caption{场点$B$在中心点上侧时电场强度情况}
        \end{center}
    \end{figure}\\
    场点$B$在中心下侧时的电场强度情况：
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=1.0]
                ...
                \tkzMarkAngle[size=0.491](Q1,Q2,B)
                \tkzMarkAngle[size=0.491](Q1,B,E-)
                ...
                \node at(+2.3,-0.2){$\theta$};
            \end{tikzpicture}
            \caption{场点$B$在中心点下侧时电场强度情况}
        \end{center}
    \end{figure}\\
    场点$B$无论在中心点上侧还是下侧，其电场强度方向均是向左的。\\[3mm]
    场点$B$处的电场强度大小为：$E_B=\left(E^++E^-\right)\cdot\cos{\theta}$。
...
\newpage
    ...
    电偶极子轴线的中垂线上的电场强度（精确值）：
    \begin{large}
        \begin{equation*}
            \veb{E_B}=-\frac{1}{4\pi\cdot\varepsilon_0}\cdot\frac{\veb{p}}{\left(r^2+\frac{l^2}{4}\right)^{\frac{3}{2}}}
        \end{equation*}
    \end{large}\\
    电偶极子轴线的中垂线上的电场强度（近似值）：
    \begin{large}
        \begin{equation*}
            \veb{E_B}=-\frac{1}{4\pi\cdot\varepsilon_0}\cdot\frac{\veb{p}}{r^3}
        \end{equation*}
    \end{large}\\
    电偶极子轴线的中垂线上的电场强度的函数图像：
    \begin{figure}[h!]
        \begin{center}
            \begin{tikzpicture}
                \tkzDefPoint(0,0){O}
                ...
                \tkzLabelPoint[above left](Q1){$-q$}
                \tkzLabelPoint[above right](Q2){$+q$}

                \tkzLabelPoint[above](X2){$r$}
                \tkzLabelPoint[right](Y2){$\veb{E_B}$}

                \draw[smooth,thick,samples=100,domain=-5:+5] plot(\x,{-0.5*((1)/(\x*\x+0.5)^1.5)});

                \draw[smooth,gray,thick,samples=100,domain=+0.46:+5] plot(\x,{-0.5*((1)/(\x)^3)});
                \draw[smooth,gray,thick,samples=100,domain=-5:-0.46] plot(\x,{+0.5*((1)/(\x)^3)});
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                ...
                \tkzDrawSegment(Z1,Z0)
                \tkzDrawSegment[dashed](Z0,O)
                \tkzDrawSegment[->](O,Z2)

                \tkzLabelPoint[right](X2){$X$}
                \tkzLabelPoint[above](Y2){$Y$}
                \tkzLabelPoint[right](Z2){$Z$}

                \draw[very thick] (0,0) ellipse (4 and 2);
                \draw[smooth,line width=6pt,samples=100,domain=-5:5] plot({4*cos(\x)},{2*sin(\x)});

                \tkzDefPoint(4,0){A}
                \tkzDefPoint(0,4){P}

                \tkzDefShiftPoint[P](90:1){EZ}
                \tkzDefShiftPoint[P](180:1){EY}
                \tkzDefShiftPoint[P](135:1.3){E}

                \tkzDrawSegment[dashed](A,P)
                \tkzDrawSegment[thick,-latex](P,EZ)
                \tkzDrawSegment[thick,-latex](P,EY)
                \tkzDrawSegment[thick,-latex](P,E)

                \tkzMarkAngle[size=0.3213](O,P,A)

                \tkzLabelPoint[right](EZ){$\dif\veb{E_\parallel}$}
                \tkzLabelPoint[above left](E){$\dif\veb{E}$}
                \tkzLabelPoint[left](EY){$\dif\veb{E_\perp}$}

                \tkzLabelPoint[above right](A){$\dif l$}

                \tkzLabelSegment[below](O,A){$a$}
                \tkzLabelSegment[left,pos=0.4](O,P){$r$}

                \tkzLabelPoint[above right](P){$P$}

                \tkzDrawPoints[fill=white](O,P)
        ...
        \end{center}
    \end{figure}\\
    假设一带电圆环，其半径为$a$，其电荷的线密度为$\lambda$，其电荷量为$Q$。\\[3mm]
    显然该带电圆环的电荷线密度$\lambda$和电荷量$Q$有以下关系：\vspace{3pt}
    \begin{align}
        Q=2\cdot\pi\cdot a\cdot\lambda
    \end{align}\\
    现在考虑带点圆环的轴上至圆心距离为$r$的一点$P$处的电场强度。
    ...
    \begin{figure}[h!]
        \begin{center}
            \begin{tikzpicture}
                ...
                \tkzDefPoint(-0.707,0){Q1}
                \tkzDefPoint(+0.707,0){Q2}

                \tkzDefShiftPoint[Q1](0,-6.5){Q1A}
                \tkzDefShiftPoint[Q1](0,+6.5){Q1B}

                \tkzDefShiftPoint[Q2](0,-6.5){Q2A}
                \tkzDefShiftPoint[Q2](0,+6.5){Q2B}

                \tkzLabelPoint[above left](Q1){$-\dfrac{\sqrt{2}a}{2}$}
                \tkzLabelPoint[above right](Q2){$\dfrac{\sqrt{2}a}{2}$}

                \tkzLabelPoint[above](X2){$r$}
                \tkzLabelPoint[right](Y2){$\veb{E}$}

                \draw[smooth,thick,samples=100,domain=-6:+6] plot(\x,{5*\x/(\x*\x+1)^1.5});
                ...
                \tkzLabelPoint[below left](O){$O$}
            \end{tikzpicture}
            \caption{均匀带电圆环轴线上的电场强度的函数曲线}
        \end{center}
    \end{figure}\\
    均匀带电圆环轴线上的电场强度的大小左侧在$r=-\dfrac{\sqrt{2}a}{2}$时取最大值。\\[3mm]
    均匀带电圆环轴线上的电场强度的大小右侧在$r=+\dfrac{\sqrt{2}a}{2}$时取最大值。

\newpage

\subsubsection{均匀带电圆盘轴线上的电场强度}
    \setcounter{equation}{0}
    均匀带电圆盘轴线上的电场强度：
    \begin{large}
        \begin{equation*}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                \draw[fill=LightGray] (0,0) ellipse (4 and 2);
                ...
                \draw[very thick] (0,0) ellipse (4 and 2);
                \draw[line width=3pt] (0,0) ellipse (1.5 and 0.75);

                \tkzDefPoint(4,0){A}
                \tkzDefPoint(0,4){P}
                \tkzDefPoint(-1.5,0){B}

                \tkzDefShiftPoint[P](90:1.2){E}

                \tkzDrawSegment[dashed](B,P)
                \tkzDrawSegment[thick,-latex](P,E)

                \tkzLabelPoint[right](E){$\dif\veb{E}$}

                \tkzLabelPoint[above left](B){$\dif x$}

                \tkzLabelSegment[below](O,A){$a$}
                \tkzLabelSegment[below,pos=0.45](O,B){$x$}
                \tkzLabelSegment[left,pos=0.4](O,P){$r$}
    ...
    \begin{figure}[h!]
        \begin{center}
            \begin{tikzpicture}
                ...
                \draw[smooth,thick,samples=100,domain=0:+6] plot(\x,{5.3*(1-1*\x/(\x*\x+1)^0.5)});
                \draw[smooth,thick,samples=100,domain=-6:0] plot(\x,{-5.3*(1+1*\x/(\x*\x+1)^0.5)});
                ...
                \tkzDrawPoints[fill=white](Q1,Q2,P1,P2)
                \tkzLabelPoint[below left](O){$O$}
            \end{tikzpicture}
            \caption{均匀带电圆盘轴线上的电场强度的函数曲线}
        \end{center}
    \end{figure}\\
    均匀带电圆盘轴线上的电场强度在$r=0$处从左右各逼近一个定值。\\[3mm]
    均匀带电圆盘轴线上的电场强度在$r=0$处不连续。
...
\subsection{电通量}
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            {
                \begin{tikzpicture}[scale=0.8]
                    \tkzDefPoint(1,-1.6){SA1}
                    \tkzDefPoint(1,+1.6){SA2}

                    \tkzDefPoint(2,-2.6){SB1}
                    \tkzDefPoint(2,+2.6){SB2}

                    \tkzDrawSegment[color=DarkGray,ultra thick](SA1,SA2)
                    \tkzDrawSegment[color=DarkGray,ultra thick](SB1,SB2)

                    \foreach \i in {-3,-2.0,...,3}
                    {
                        \tkzDefPoint(-3,\i){A}
                        \tkzDefPoint(+3,\i){B}
                        \tkzDrawSegment[-latex](A,B)
        ...
        \end{center}
    \end{figure}\\
    当曲面面积一定时，电场强度越大，通过曲面的电场线数量越多，表明电通量越大。\\[3mm]
    当电场强度一定时，曲面面积越大，通过曲面的电场线数量越多，表明电通量越大。
...
\subsubsection{高斯电场定理}
    ...
    \begin{figure}[h]
        \begin{center}
            \subfigure[正电荷的情况]
            {
                \begin{tikzpicture}[scale=0.8]
                    ...
                    \tkzDefPoint(2,0){S}
                    \tkzDrawCircle[color=DarkGray,ultra thick](O,S)
    
                    \foreach \i in {0,15,...,345}
                    {
                        \tkzDefPoint(\i:1.45){A1}
                        \tkzDefPoint(\i:3.0){A2}
                        \tkzDrawSegment[-latex](O,A1)
                        \tkzDrawSegment(A1,A2)
                    }
        ...
        \end{center}
    \end{figure}\\
    假设一个点电荷$Q$，取以电荷为球心半径为$r$的球面作为高斯面，研究该闭合曲面上的电通量。\\[3mm]
    因为电场线总是和面积元垂直：
    \begin{align}
        \cos{\theta}=1
    \end{align}\\
    因此球面上的电通量可以写作：
    \begin{align}
        &\Phi_E=\oint_S E\cdot\dif S\cdot\cos{\theta}\\[3mm]
        &\Phi_E=\oint_S E\cdot\dif S
    \end{align}\\
    接下来将以此推导通过该球面的电通量。
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}[scale=0.6]
                ...
                \draw[color=DarkGray,ultra thick] circle(2);

                \foreach \i in {0,15,...,345}
                {
                    \tkzDefShiftPoint[O](\i:3.0){A1}
                    \tkzDefShiftPoint[O](\i:7.0){A2}
                    \tkzDrawSegment[-latex](O,A1)
                    \tkzDrawSegment(A1,A2)
                }

                \tkzDefShiftPoint[O](0.4,0){R}
                \tkzDrawCircle[fill=white](O,R)
        ...
        \begin{center}
            \subfigure[场点在球面内部的情况]
            {
                \begin{tikzpicture}[scale=0.65]
                    ...
                    \foreach \i in{50,60,...,130}
                    {
                        \node at(\i:3.3) {\scriptsize $+$};
                    }

                    \draw[dashed] (-4.7,-4.5) rectangle (4.7,4.5);

                    \tkzDrawSegment(O,C)
                    \tkzDrawSegment(O,S)

                    \tkzLabelSegment[above right](O,C){$R$}
                    \tkzLabelSegment[above](O,S){$r$}

                    \tkzLabelPoint[right](S){$P$}
                    \tkzLabelPoint[below](O){$O$}

                    \draw[color=DarkGray,ultra thick] (0,0) circle(2.1);
                    \draw[line width=4pt] (0,0) circle(3.0);

                    \tkzDrawPoints[fill=white](O,S,C)
                \end{tikzpicture}
            }\qquad
            \subfigure[场点在球面外部的情况]
            {
                \begin{tikzpicture}[scale=0.65]
                    \tkzDefPoint(0,0){O}
                    ...
                    \draw[color=DarkGray,ultra thick] (0,0) circle(3.7);
                    \draw[line width=4pt] (0,0) circle(3);
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            \begin{tikzpicture}[scale=1.15]
                ...
                \draw[very thick,domain=1:9,samples=100] plot(\x,{4/\x^2});
                ...
                \tkzDrawPoints[fill=white](A,C)

                \tkzLabelPoint[below](O){$O$}
                \tkzLabelPoint[below](A){$R$}
                \tkzLabelPoint[above](X){$r$}
                \tkzLabelPoint[right](Y){$E$}
            \end{tikzpicture}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                ...
                \tkzDefShiftPoint[E1](0,-1.5){L1}
                \tkzDefShiftPoint[E2](0,+1.5){L2}

                \tkzDefShiftPoint[L1](-1,0){L1A}
                \tkzDefShiftPoint[L1](+1,0){L1B}

                \tkzDefShiftPoint[L2](-1,0){L2A}
                \tkzDefShiftPoint[L2](+1,0){L2B}

                \tkzDrawSegment[line width=3pt](E1,E2)

                \begin{scope}
                    \tkzDefShiftPoint[L1A](0,+3){L1A+}
                    \clip (L1A+) rectangle (L1B);
                    \fill[LightGray,fill opacity=0.5] (L1) ellipse (1 and 0.5);
                \end{scope}

                \begin{scope}
                    \tkzDefShiftPoint[L2A](0,-3){L2A+}
                    \clip (L2A+) rectangle (L2B);
                    \fill[LightGray,fill opacity=0.5] (L2) ellipse (1 and 0.5);
                \end{scope}
                
                \fill[LightGray,fill opacity=0.5] (L1A) rectangle (L2B);

                \tkzDrawSegment[line width=3pt](E1,L1)

                \tkzDrawSegment(L1,L1B)
                \tkzDrawSegment[dashed](L2,L2B)

                \draw (L1A)--(L2A);
                \draw (L1B)--(L2B);

                \draw[smooth,samples=100,domain=0:180] plot({1*cos(\x)},{0.5*sin(\x)+3});
                \draw[smooth,samples=100,domain=180:360] plot({1*cos(\x)},{0.5*sin(\x)+3});

                \draw[smooth,samples=100,domain=0:180,dashed] plot({1*cos(\x)},{0.5*sin(\x)-3});
                \draw[smooth,samples=100,domain=180:360] plot({1*cos(\x)},{0.5*sin(\x)-3});

                \draw[smooth,samples=100,domain=0:180,dashed] plot({1*cos(\x)},{0.5*sin(\x)});
                \draw[smooth,samples=100,domain=180:360] plot({1*cos(\x)},{0.5*sin(\x)});

                \tkzLabelPoint[right](E1){$\lambda$}
                \tkzLabelSegment[above](L1,L1B){$r$}
                \tkzLabelSegment[above](L2,L2B){$r$}
                \tkzLabelSegment[right,pos=0.6](L1B,L2B){$h$}

                \tkzDrawPoints[fill=white](L1,L2,O,L1B)

                \tkzLabelPoint[right](L1B){$P$}
            \end{tikzpicture}
            \caption{无限长均匀带电直线的电场强度的示意图}
        \end{center}
    \end{figure}\\
    假设一线密度为$\lambda$的无限长带电直线，以其为轴取半径为$r$高度为$h$的圆柱面作高斯面。\\[3mm]
    假设一场点$P$至轴的距离为$r$，即场点在高斯面上，现研究该场点的电场强度情况。\\[3mm]
    此时高斯面内的电荷为$\lambda\cdot h$：
    \begin{align}
        \Phi_E=\frac{\lambda\cdot h}{\varepsilon_0}
    \end{align}\\
    该高斯面上，圆柱的两个底面的电通量为零，圆柱的侧面占有了所有的电通量。
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            \begin{tikzpicture}[scale=1.15]
                ...
                \draw[very thick,domain=0.9:9,samples=100] plot(\x,{4/\x});
...
\subsubsection{无限大均匀带电平面的电场强度}
    ...
    \begin{figure}[h]
        \begin{center}
            \begin{tikzpicture}
                \tkzDefPoint(+3.414,+1.414){A1}
                \tkzDefPoint(+0.586,-1.414){A2}
                \tkzDefPoint(-3.414,-1.414){A3}
                \tkzDefPoint(-0.586,+1.414){A4}

                \tkzDrawPolygon[fill=MediumGray](A1,A2,A3,A4)
                ...
                \tkzInterLL(A1,A2)(Y1,Y2)
                \tkzGetPoint{A12}

                \tkzInterLL(A3,A4)(Y1,Y2)
                \tkzGetPoint{A34}
                ...
                \tkzDrawPolygon[fill=MediumGray](A2,A3,A34,A12)
                \tkzDrawSegment(O,X2)
                \tkzDrawSegment[color=gray](O,L2)

                \begin{scope}
                    \tkzClipPolygon(A2,A3,A34,A12)
                    \fill[LightGray,fill opacity=0.5] (O) ellipse (1 and 0.5);
                \end{scope}
                ...
                \tkzLabelSegment[right,pos=0.85](L1B,L2B){$r$}
                \tkzLabelSegment[right,pos=0.85](L2B,L1B){$r$}

                \tkzDrawPoints[fill=white](L1,L2,O,L1B)

                \tkzLabelPoint[right](L1B){$P$}
                \tkzLabelPoint[left](A3){$\sigma$}
            \end{tikzpicture}
            \caption{无限大均匀带电平面的电场强度的示意图}
        \end{center}
    \end{figure}\\
    假设一面密度为$\sigma$的无限大带电平面，以法线为轴取高度为$2r$底面积为$S$的圆柱面作高斯面。\\[3mm]
    假设一场点$P$至平面的距离为$r$，即场点在高斯面上，现研究该场点的电场强度情况。\\[3mm]
    此时高斯面内的电荷为$\sigma\cdot S$：
    \begin{align}
        \Phi_E=\frac{\sigma\cdot S}{\varepsilon_0}
    \end{align}\\
    该高斯面上，圆柱的侧面的电通量为零，圆柱的两个底面占有了所有的电通量。
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            \begin{tikzpicture}[scale=1.15]
                ...
                \tkzDrawSegment[very thick](A1,A2)
